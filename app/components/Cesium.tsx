...
export default function Cesium({ }: GlobeComponentProps) {
    ...
    const { mapLayerOpacity, pickedMapLayer, radios, currentRadio, setCurrentRadio, currentTheme, changeRadioFromOutsideGlobe, capitals } = useAppStore();
    ...
    const zoomToRadio = (radio: RadioStation) => {
        ...
        if (viewer) {
            ...
            if(radio.geo_lat === null && capitals) {
    ...
    useEffect(() => {
        if (radios.length) {
            const entities: JSX.Element[] = radios.filter(r => r.geo_lat && r.geo_long)
                .map((r) => {
    ...
    useEffect(() => {
        ...
        if(currentRadio ) {
            if ( currentRadio.geo_long && currentRadio.geo_lat) {
